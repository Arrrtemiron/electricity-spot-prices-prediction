...
generated quantities {
  ...
  }
  {
    y_pred_train[1] = y[1];
    for (i in 2:N-1) {
        y_pred_train[i] = y[i-1] + mu + d_sat * D_Sat[i] + d_sun * D_Sun[i] + d_mon * D_Mon[i] + sqrt(exp(h[i-1])) * eps1[i] + jumps[i] + psi_2 * min_y[i];
    }
  }
  prediction = y_pred_train[N-1] + mu + d_sat * D_Sat[N] + d_sun * D_Sun[N] + d_mon * D_Mon[N] + sqrt(exp(h[N-1])) * eps1[N] + jumps[N] + psi_2 * min_y[N];
}